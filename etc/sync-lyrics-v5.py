...
Mainu tere naa' pyaar jatta taan) and Devanagari (Hindi) into casual
"Hinglish" (e.g. मिल गई है तेरी भूरी-भूरी -> mil gai hai teri bhuri-bhuri),
unless you pass --no-romanize.
...
Usage:
    ...
    ./sync-lyrics.py --json             Output as JSON (per-line in watch mode, full in --once)
    ./sync-lyrics.py --no-romanize      Keep Gurmukhi/other non-Latin script as-is
    ./sync-lyrics.py --backends lrclib  Only use LRCLIB (skip local files + LrcMux)
...
def _gm_word(word):
    return _render(_gm_apply_rules(_gm_tokenize(word)))
...
_DV_NUKTA_VARIANTS = {"क": "q", "ख": "kh", "ग": "g", "ज": "z",
                      "ड": "r", "ढ": "rh", "फ": "f", "य": "y"}
_DV_INDEP_VOWELS = {
...
_DV_MATRAS = {
    ...
    "ृ": "ri", "ॄ": "rree", "े": "e", "ै": "ai", "ो": "o", "ौ": "au",
    "ॅ": "a", "ॆ": "e", "ॉ": "o", "ॊ": "o",
}
_DV_IRREGULAR = {"गई": "gai", "गए": "gaye"}
...
def _dv_tokenize(word):
    ...
    while i < n:
        ...
        if ch in _DV_CONSONANTS:
            ...
            if i + 1 < n and chars[i + 1] == _DV_NUKTA and ch in _DV_NUKTA_VARIANTS:
                base = _DV_NUKTA_VARIANTS[ch]
                i += 1

            vowel, is_matra, nasal = "a", False, None
            j = i + 1
            if j < n and chars[j] in _DV_MATRAS:
                vowel, is_matra = _DV_MATRAS[chars[j]], True
                i = j
            ...
            k = i + 1
            if k + 1 < n and chars[k] in _DV_NASAL_MARKS and chars[k + 1] in _DV_MATRAS:
                nasal = chars[k]
                vowel, is_matra = _DV_MATRAS[chars[k + 1]], True
                i = k + 1
        ...
        if ch == _DV_VISARGA:
            syllables.append(dict(base=None, vowel=None, is_matra=False,
                                   nasal=None, indep=False, literal="h"))
            i += 1
            continue

        if ch == _DV_AVAGRAHA:
            syllables.append(dict(base=None, vowel=None, is_matra=False,
                                   nasal=None, indep=False, literal="'"))
            i += 1
            continue

        if ch in _DV_NASAL_MARKS:
            syllables.append(dict(base=None, vowel=None, is_matra=False,
                                   nasal=None, indep=False, literal="n"))
            i += 1
            continue
...
def _dv_apply_rules(syllables):
    for idx, s in enumerate(syllables):
        ...
        if nxt is not None and nxt["literal"] is None and nxt["base"] in _NASAL_BASES:
            s["_append_n"] = False
        else:
            s["_append_n"] = True
    ...
    for idx in range(1, len(syllables)):
        cur, prev = syllables[idx], syllables[idx - 1]
        if (cur["indep"] and cur["vowel"] == "aa" and
                prev["literal"] is None and prev["is_matra"] and prev["vowel"] == "i"):
            cur["vowel"] = "ya"

    if syllables and syllables[-1]["literal"] is None:
        last = syllables[-1]
        if last["vowel"] == "aa" and not last["nasal"] and (
                last["is_matra"] or last["indep"]):
            last["vowel"] = "a"
...
def _dv_word(word):
    ...
    return _render(_dv_apply_rules(_dv_tokenize(word)))
...
def maybe_romanize(text, enabled):
    ...
    if enabled and _is_devanagari(text):
...
def try_lrcmux(meta):
    """Aggregate lrcmux (free): fans out across Musixmatch/KuGou/ytmusic/Genius,
    NetEase excluded; returns line-level synced LRC or None."""
    params = {"artist": meta["artist"], "title": meta["title"],
    ...
    if meta["duration"] > 0:
        params["duration"] = round(meta["duration"])
    try:
        r = requests.get(LRCMUX_GET, params=params,
                         headers=LRCMUX_HEADERS, timeout=8)
...
def notify(title, body):
    run(["notify-send", "-a", "sync-lyrics", "-u", "critical",
         "-r", "9271", "-t", "0", title, body])
    ...
    while True:
        ...
        if key != last_key:
            ...
            if not lrc_text:
                ...
                if args.json:
                    # Emit track-start event with metadata
                    print(json.dumps({
                        "event": "track_start",
                        "metadata": meta,
                        "source": source,
        ...
        if idx != printed_idx and idx >= 0:
            ...
            if args.json:
                print(json.dumps({
                    "event": "lyric",
                    "time": lines[idx][0],
                    "formatted_time": format_time(lines[idx][0]),
                    "text": text,
                    "index": idx,
                    "artist": meta["artist"],
                    "title": meta["title"]
...
def once(player, args):
    ...
    if not meta:
        if args.json:
            print(json.dumps({"error": "No track currently playing"}))
        else:
...
def main():
    ...
    parser.add_argument("--json", action="store_true", help="output as JSON (per-line in watch mode, full object in --once)")
    parser.add_argument("--player", help="MPRIS player name (defaults to the first one playerctl finds)")
    parser.add_argument("--backends", default="local,lrclib,lrcmux",
                         help="comma-separated backend order to try: local,lrclib,lrcmux (default: all three)")
    ...
    parser.add_argument("--no-romanize", action="store_true",
                         help="don't transliterate Gurmukhi/Devanagari lyrics to Latin script")
    ...
    if not player:
        if args.json:
            print(json.dumps({"error": "No MPRIS-capable player found"}))
        else:
