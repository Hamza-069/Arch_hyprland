...
# Global reference to player manager for signal handler
_player_manager_ref = None


def signal_handler(sig, frame):
    logger.info("Received signal to stop, exiting")
    global _player_manager_ref
    if (
        _player_manager_ref
        and hasattr(_player_manager_ref, "timeout_id")
        and _player_manager_ref.timeout_id
    ):
        GLib.source_remove(_player_manager_ref.timeout_id)
    sys.stdout.write("\n")
    sys.stdout.flush()
    # loop.quit()
    sys.exit(0)
...
class PlayerManager:
    ...
    def run(self):
        ...
        # Set up a timeout to update every 2 seconds
        self.timeout_id = GLib.timeout_add_seconds(3, self._on_timeout)
        self.loop.run()
